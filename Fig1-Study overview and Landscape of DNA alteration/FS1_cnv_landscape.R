library(openxlsx)
# read data ################################
df.all = readRDS('~/Dropbox/HOPE_iProFun/github/pediatric_aya_hgg_study_data.rds')

clinical = df.all$clinical
cnv = df.all$cnv$data
g.cnv = df.all$cnv$anno$ApprovedGeneSymbol

# gene_loc = readRDS('~/Dropbox/HOPE_iProFun/harmonized_gene_location_10232023_with_arm.Rds')
# write.csv(gene_loc[,c(1:5,7:9)],'~/Dropbox/HOPE_iProFun/github/harmonized_gene_location_10232023.csv',row.names = F)
